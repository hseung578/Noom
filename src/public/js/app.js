...
function handleMuteClick() {
  ...
  if (!muted) {
    muteBtn.classList.remove("btnColor");
    muted = true;
  } else {
    muteBtn.classList.add("btnColor");
    muted = false;
  }
}
function handleCameraClick() {
  myStream
    ...
    .forEach((track) => (track.enabled = !track.enabled));
  if (cameraOff) {
    cameraBtn.classList.add("btnColor");
    cameraOff = false;
  } else {
    cameraBtn.classList.remove("btnColor");
    cameraOff = true;
  }
}
...
function sendPeersMessage(msg) {
  ...
  if (msg.data.length > 31) {
    div.innerText = `Peer: ${msg.data}`;
    li.appendChild(div);
  } else {
    span.innerText = `Peer: ${msg.data}`;
    li.appendChild(span);
  }
  ul.appendChild(li);
  if (messageList.scrollHeight >= message.clientHeight - 70) {
...
function handleMessageSubmit(event) {
  ...
  sendMyMessage(messageText);
  if (myDataChannel !== "") {
    myDataChannel.send(messageText);
  }
  input.value = "";
  if (messageList.scrollHeight >= message.clientHeight - 70) {
    messageList.scrollTop = messageList.scrollHeight;
...
socket.on("welcome", async () => {
  ...
  myDataChannel.addEventListener("message", sendPeersMessage);
